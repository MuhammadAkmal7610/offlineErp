...
export default function Sales() {
  ...
  useEffect(() => {
    ...
    loadBalance();
  }, [selectedStudent?.id, activeBusiness]);
            ...
            <tbody>
              ${salesList?.map((s) => `
                <tr>
                  ...
                  <td>${currency} ${s.totalAmount.toFixed(2)}</td>
                </tr>
              `).join('')}
              <tr class="total">
    ...
    <div className="space-y-6">
      ...
      <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-panel">
        <div className="flex gap-3 mb-4 items-center flex-wrap">
          ...
          <button onClick={() => {
            const currentDB = getDB(activeBusiness);
            currentDB.sales.toArray().then(salesData => {
              currentDB.students.toArray().then(studentData => {
                loadSalesList(salesData, studentData);
              });
            });
