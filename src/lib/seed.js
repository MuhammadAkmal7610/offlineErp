...
export async function seedDatabase(targetDB, businessType = 'general') {
  ...
  const productIds = await Promise.all(sampleProducts.map((product) => targetDB.products.add(product)));
  ...
  if (businessType === 'general') {
    const studentIds = await Promise.all(sampleStudents.map((student) => targetDB.students.add(student)));
    const studentLedgerWithIds = [
      { ...sampleStudentLedger[0], studentId: studentIds[0] },
      { ...sampleStudentLedger[1], studentId: studentIds[0] },
      { ...sampleStudentLedger[2], studentId: studentIds[0] },
      { ...sampleStudentLedger[3], studentId: studentIds[1] },
    ];

    await Promise.all([
      targetDB.inventory.bulkAdd(inventoryItems),
      targetDB.suppliers.bulkAdd(sampleSuppliers),
      targetDB.expenses.bulkAdd(sampleExpenses),
      targetDB.purchases.bulkAdd(samplePurchases),
      targetDB.sales.bulkAdd(sampleSales),
      targetDB.studentLedger.bulkAdd(studentLedgerWithIds),
      targetDB.settings.bulkPut(settings),
    ]);
  } else {
    const customerIds = await Promise.all(sampleCustomers.map((customer) => targetDB.customers.add(customer)));
    const customerLedgerWithIds = [
      { ...sampleCustomerLedger[0], customerId: customerIds[0] },
      { ...sampleCustomerLedger[1], customerId: customerIds[0] },
      { ...sampleCustomerLedger[2], customerId: customerIds[1] },
    ];
    ...
    await Promise.all([
      ...
      targetDB.purchases.bulkAdd(samplePurchases),
      targetDB.sales.bulkAdd(sampleSales),
      targetDB.customerLedger.bulkAdd(customerLedgerWithIds),
      targetDB.settings.bulkPut(settings),
    ]);
  }
}